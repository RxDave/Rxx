...
{
  internal sealed class AllManyObservableParser<TSource, TResult> : IObservableParser<TSource, IObservable<TResult>>
  {
    ...
    #region Constructors
    public AllManyObservableParser(IEnumerable<IObservableParser<TSource, IObservable<TResult>>> parsers)
    {
                ...
                parser => (Func<Tuple<IObservableCursor<TSource>, RefCountDisposable>, Tuple<IParseResult<IObservable<TResult>>, bool>, Tuple<Tuple<IObservableCursor<TSource>, RefCountDisposable>, IObservable<IParseResult<IObservable<TResult>>>>>)
                  ((context, value) =>
                  {
                    ...
                    if (value.Item2)
                    {
                      ...
                      results = parser.Parse(branch).Finally(refDisposable.Dispose);
                    }
                    else
                    {
                      ...
                      results = parser.Parse(branch).Finally(disposable.Dispose);
                    }

...
namespace Rxx.Parsers.Linq
{
  public static partial class Parser
  {
    ...
    public static IParser<TSource, TResult> Success<TSource, TIntermediate, TResult>(
      ...
      int length)
    {
      ...
      return parser.Yield("Success", source => ParseResult.Return(result, length));
    }
    ...
    /// of zero, starting from the index at which the specified <paramref name="parser"/> starts.</returns>
    public static IParser<TSource, IEnumerable<TResult>> Success<TSource, TResult>(
      this IParser<TSource, TResult> parser)
    {
      Contract.Requires(parser != null);
      Contract.Ensures(Contract.Result<IParser<TSource, IEnumerable<TResult>>>() != null);

      return parser.Yield("Success", source => ParseResult.ReturnSuccessMany<TResult>(length: 0));
    }
    ...
    /// <param name="parser">The parser for which a single empty result sequence is returned to indicate success.</param>
    /// <remarks>
    /// <see cref="Success{TSource,TResult}(IParser{TSource,IEnumerable{TResult}})"/> is required as an explicit overload
    /// because the meaning of the parser's result sequence is special and must not be compounded into a sequence of sequences, 
    /// which would happen if the <see cref="Success{TSource,TResult}(IParser{TSource,TResult})"/> overload were to be called
    /// instead.
    /// </remarks>
    /// <returns>A parser that returns a single result containing an empty sequence with a length 
    /// of zero, starting from the index at which the specified <paramref name="parser"/> starts.</returns>
    ...
    public static IParser<TSource, TResult> Failure<TSource, TResult>(
      this IParser<TSource, TResult> parser)
    {
      Contract.Requires(parser != null);
      Contract.Ensures(Contract.Result<IParser<TSource, TResult>>() != null);

      return parser.Yield("Failure", source => ParseResult.ReturnFailure<TResult>());
    }
  }
}

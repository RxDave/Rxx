...
{
  internal sealed class AllManyUnorderedParser<TSource, TResult> : IParser<TSource, IEnumerable<TResult>>
  {
    ...
    public IEnumerable<IParser<TSource, IEnumerable<TResult>>> Parsers
    {
      get
      {
        Contract.Ensures(Contract.Result<IEnumerable<IParser<TSource, IEnumerable<TResult>>>>() != null);
    ...
    private readonly IEnumerable<IParser<TSource, IEnumerable<TResult>>> parsers;
    private IParser<TSource, IEnumerable<TResult>> firstParser;
    #endregion

    #region Constructors
    public AllManyUnorderedParser(IEnumerable<IParser<TSource, IEnumerable<TResult>>> parsers)
    {
    ...
    public IEnumerable<IParseResult<IEnumerable<TResult>>> Parse(ICursor<TSource> source)
    {
      ...
      bool first = true;
      var any = new AnyParser<TSource, IEnumerable<TResult>>(parsers);
      var except = new List<IParser<TSource, IEnumerable<TResult>>>();
              ...
              parser => (Func<Tuple<IParseResult<IEnumerable<TResult>>, bool>, IEnumerable<IParseResult<IEnumerable<TResult>>>>)
                (value =>
                {
                  ...
                  if (value.Item2)
                  {
                    ...
                    return any.Parse(except, branch);
                  }
                  else
                  {
